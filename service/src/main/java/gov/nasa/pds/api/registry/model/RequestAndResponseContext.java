...
public class RequestAndResponseContext implements RequestBuildContext, RequestConstructionContext {
	...
	static public RequestAndResponseContext buildRequestAndResponseContext(ControlContext connection, // webby
																										// criteria
			UserContext parameters, Pagination<String> lidvids)
			throws ApplicationTypeException, LidVidNotFoundException, IOException, UnknownGroupNameException {
		GroupConstraint any = ReferencingLogicTransmuter.Any.impl().constraints();
		/**
		 * The line in this comment block is valid back when referencing was used and
		 * the user told us what group they wanted explicitly. With members, this is no
		 * longer true and the group is actually wrong in most cases. GroupConstraint
		 * preset =
		...
		SearchRequest request = new SearchRequestFactory(RequestConstructionContextFactory.given(lidvids.page()),
				connection.getConnection()).build(response, connection.getConnection().getRegistryIndex());
		request.source().size(lidvids.size());
		response.setResponse(
				connection.getConnection().getRestHighLevelClient().search(request, RequestOptions.DEFAULT).getHits(),
				null, lidvids.total());
		return response;
	}
	...
	private RequestAndResponseContext(ControlContext controlContext, // webby criteria
		...
		ProductVersionSelector versionSelectionScope = outPreset.equals(resPreset) ? parameters.getSelector()
				: ProductVersionSelector.TYPED;
		...
		this.productIdentifier = LidVidUtils.resolve(parameters.getIdentifier(), versionSelectionScope, controlContext,
				RequestBuildContextFactory.given(parameters.getSelector() == ProductVersionSelector.LATEST, fields,
						resPreset));
	...
	private List<String> add_output_needs(List<String> given) throws ApplicationTypeException {
		...
		if (this.formatters.containsKey(this.format)) {
			...
			max_needs = SearchUtil
					.jsonPropertyToOpenProperty(this.formatters.get(this.format).getMaximallyRequiredFields());
			min_needs = SearchUtil
					.jsonPropertyToOpenProperty(this.formatters.get(this.format).getMinimallyRequiredFields());
		} else {
		...
		/*
		 * if the URL contains fields, then make sure the minimum was included too OR
		 * there is maximum set.
		 */
		...
		if (this.isSingular()) {
			...
			if (hits != null && hits.getTotalHits() != null) {
				...
				if (hitCount == 1L) {
					...
					String basicErrMsg = "Got " + hitCount
							+ " hits for a query which should have returned a singular result. "
