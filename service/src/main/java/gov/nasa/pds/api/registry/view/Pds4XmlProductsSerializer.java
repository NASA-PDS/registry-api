...
public class Pds4XmlProductsSerializer extends AbstractHttpMessageConverter<Pds4Products> {
	/***
	 * OBSOLETE since we don't want to use the label in blob anymore to provide the
	 * pds4 original label for a list of products
	 * 
	 */
	...
	protected void writeInternal(Pds4Products products, HttpOutputMessage outputMessage)
		...
		try {
			...
			writer.setPrefix(Pds4XmlProductSerializer.NAMESPACE_PREFIX, Pds4XmlProductSerializer.NAMESPACE_URL);
			writer.writeStartElement(Pds4XmlProductSerializer.NAMESPACE_URL, "products");
			writer.writeNamespace(Pds4XmlProductSerializer.NAMESPACE_PREFIX, Pds4XmlProductSerializer.NAMESPACE_URL);
			...
			this.logger.error(
					"For XML serialization, Product object must be extended to ProductWithXmlLabel: " + e.getMessage());
		} catch (Exception e) {
