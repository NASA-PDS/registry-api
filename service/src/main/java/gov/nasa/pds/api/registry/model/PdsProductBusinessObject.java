...
public class PdsProductBusinessObject implements ProductBusinessLogic {
	...
	public void setResponse(SearchHit hit, List<String> fields) {
		...
		product = SearchUtil.entityProductToAPIProduct(objectMapper.convertValue(kvp, EntityProduct.class),
				this.baseURL);

		product.setProperties((Map<String, List<String>>) ProductBusinessObject.getFilteredProperties(kvp, null, null));
	...
	public int setResponse(HitIterator hits, Summary summary, List<String> fields) {
		...
		for (Map<String, Object> kvp : hits) {
			...
			products.addDataItem(SearchUtil
					.entityProductToAPIProduct(objectMapper.convertValue(kvp, EntityProduct.class), this.baseURL));
