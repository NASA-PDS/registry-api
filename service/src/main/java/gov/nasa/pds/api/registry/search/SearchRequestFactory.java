...
public class SearchRequestFactory {
	...
	public SearchRequestFactory(RequestConstructionContext context, ConnectionContext registry) {
		...
		if (context.getProductIdentifier() != null) {
			PdsProductIdentifier productIdentifier = PdsProductIdentifier
					.fromString(context.getProductIdentifierString());
			String key = productIdentifier instanceof PdsLidVid ? "lidvid" : "lid";

			this.base.must(QueryBuilders.termQuery(key, context.getProductIdentifierString())); // term is exact match
																								// which lidvid look
																								// should be
		}
	}
	...
	static private String[] excludes(List<String> fields) {
		...
		if (fields.contains(BlobUtil.XML_BLOB_PROPERTY) && fields.contains(BlobUtil.JSON_BLOB_PROPERTY))
			exclude = ex0;
		else if (fields.contains(BlobUtil.XML_BLOB_PROPERTY))
			exclude = exjbp;
		else if (fields.contains(BlobUtil.JSON_BLOB_PROPERTY))
			exclude = exbp;
		else
			exclude = exall;
	...
	public SearchRequest build(RequestBuildContext context, String index) {
		...
		return new SearchRequest().indices(index).source(new SearchSourceBuilder().query(this.base).fetchSource(
				context.getFields().toArray(new String[0]), SearchRequestFactory.excludes(context.getFields())));
	}
}
