...
public class SearchRequestFactory {
  ...
  public SearchRequestFactory(RequestConstructionContext context, ConnectionContext registry) {
    ...
    if (context.getProductIdentifier() != null) {
      PdsProductIdentifier productIdentifier =
          PdsProductIdentifier.fromString(context.getProductIdentifierString());
      String key = productIdentifier instanceof PdsLidVid ? "lidvid" : "lid";

      this.base.must(QueryBuilders.termQuery(key, context.getProductIdentifierString())); // term is
                                                                                          // exact
                                                                                          // match
                                                                                          // which
                                                                                          // lidvid
  ...
  public SearchRequest build(RequestBuildContext context, String index) {
    if (this.regContext.getRegistryIndex().equals(index)) {
      log.debug("************          Just the latest lidvids: "
          + Boolean.toString(context.justLatest()));

      if (context.justLatest()) {
        ProductQueryBuilderUtil.addHistoryStopband(this.base);
        log.debug("************          created and filled the stopband:"
            + this.base.mustNot().toString());
      }
