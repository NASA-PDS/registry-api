...
public class CollectionProductIterator<T> implements Iterator<T> { 
    ...
    private Iterator<String> initProductIterator() {
    	if (this.searchHitsIterator.hasNext()) {
    		...
    		// TODO manage this edge case so that it triggers a 404 error 
    		// (To Be confirmed, since actually real 404 would be if the lidvid of the collection is not available in registry index)
    		// and here we are only seeing that there is not relationship found in the relationship collection
    		return null;
    	}
    }
} 
